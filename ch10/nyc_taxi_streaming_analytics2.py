...
SELECT
  datetime(timestamp, 'Asia/Tokyo') as datetime,
  ride_status,
  count
FROM `singular-rope-268807.nyc_taxi_trip.rides_1m`
...
class AttachWindowTimestamp(beam.DoFn):
    ...
    def process(self, element, window=beam.DoFn.WindowParam):
        ...
        status_count = {
            "timestamp": window_start_dt.strftime("%Y-%m-%d %H:%M:%S"),
            "ride_status": status,
            "count": count,
        }
        yield status_count
...
def run(argv=None, save_main_session=True):
    ...
    parser.add_argument(
        "--output_dataset",
        required=True,
        help=('Output BigQuery dataset "<PROJECT>.<DATASET>"'),
    )
